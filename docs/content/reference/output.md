...
describes what users get. Pick a format with `-o`, or let icpc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
icpc <command> -o table   # aligned columns for reading
icpc <command> -o jsonl   # one JSON object per line, for piping
icpc <command> -o json    # a single JSON array
icpc <command> -o csv     # spreadsheet friendly
icpc <command> -o tsv     # tab-separated
icpc <command> -o url     # just the URL column
icpc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
icpc <command> --fields id,title,url
```
...
```bash
icpc <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
icpc <command>            # a table, because this is a terminal
icpc <command> | wc -l    # JSONL, because this is a pipe
```
